...
Writing `farrow-api` is just like typing in a higher-order way, we define a api-type via `farrow-schema`. And then use [farrow-api-server](./09-farrow-api-server.md) to attach api to a http server.
